...
def filter_gRNA(gRNA_file, TF_dict):
    ...
    for gRNA in range(len(GenomeCoordinates)):
        ...
        int(GenomeCoordinates['fmin'].iloc[gRNA]) >= TF_dict["Start"] - 20 and \
        int(GenomeCoordinates['fmax'].iloc[gRNA]) <= TF_dict["Stop"] + 20:
                
            sgRNA_list_positions.append([int(GenomeCoordinates['fmin'].iloc[gRNA]), int(GenomeCoordinates['fmax'].iloc[gRNA])])
...
def mutate_sgRNA_recognition_site_in_HDR_plasmid(sequenceType, sequenceToMutate, positionCount):
    ...
    if mutatedCount < 2:
        ...
        if sequenceType == 'HAL-R':
            if len(mutableRegion) >20:
                mutatedSequence = sequenceToMutate[:len(mutableRegion)-21] + mutableRegion + sequenceToMutate[-positionCount:]
            else:
